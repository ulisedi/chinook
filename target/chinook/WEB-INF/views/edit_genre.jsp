<%@page import="model.Genre"%>
<%
    Genre genre = (Genre) request.getAttribute("genre");
    if (genre == null) {
        // Si no hay genero, volvemos al listado pasando por el servlet
        response.sendRedirect("genre");
        return;
    }
%>

<!DOCTYPE html>
<html lang="es">
<head>

<title>Editar genero</title>
<%@ include file="/WEB-INF/head/head.jsp" %>
</head>
<body class="bg-light">

<!-- Header/Navbar -->
<nav class="navbar">
    <a href="${pageContext.request.contextPath}/index_crud.jsp" class="navbar-brand-link">
        <img src="${pageContext.request.contextPath}/img/logo.ico" alt="Chinook Logo" class="navbar-logo">
        <span class="navbar-brand"> Chinook</span>
    </a>
    <div style="flex-grow: 1;"></div>
    
    <!-- Toggle de tema -->
    <button class="theme-toggle-navbar" title="Cambiar a modo oscuro">
        <i class="fas fa-moon"></i>
    </button>
</nav>

<div class="container mt-5">
    <h2 class="mb-4">Editar genero</h2>

    <div class="card">
        <div class="card-body">

            <form action="genre" method="post">
                <input type="hidden" name="action" value="update">
                <input type="hidden" name="id" value="<%= genre.getGenreId() %>">

                <div class="mb-3">
                    <label class="form-label">Nombre del genero</label>
                    <input type="text" name="name" class="form-control" value="<%= genre.getName() %>" required>
                </div>

                <button type="submit" class="btn btn-primary">Actualizar</button>
                <a href="genre" class="btn btn-secondary">Cancelar</a>
            </form>

        </div>
    </div>
</div>

<!-- JS de tema noche/día -->
<script src="${pageContext.request.contextPath}/js/theme-toggle.js"></script>
</body>
</html>